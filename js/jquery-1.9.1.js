...
/**
 * @author hzq 2015-01-14
 * 为什么要出入window
 * 	1、方法会寻找变量，window再此相当于局部变量，根据作用域链如果此处不定义需要向上查找，
 * 		如果在这里定义了则不用再向上查找
 * 	2、方便压缩，方法体内的window可以替换成其他名字的变量
 */
(function(window, undefined) {
	...
	jQuery.fn = jQuery.prototype = {
		...
		init: function(selector, context, rootjQuery) {
			...
			if (typeof selector === "string") {
				...
				// 能进入这个判断的是 $('<li>') 或者 ($("#xxx")并且没有上下文的，上下文会进入 ： hzq_201501142153)
				...
				if (match && (match[1] || !context)) {
					...
					// 处理$("<div/>")、$("<p>")一类
					if (match[1]) { //创建元素
						// context 主要是用在iframe等一类操作中
						context = context instanceof jQuery ? context[0] : context;

						// scripts is true for back-compat 向上兼容
						...
						if (rsingleTag.test(match[1]) && jQuery.isPlainObject(context)) {
							for (match in context) {
								...
								// $('<li>', {title:'hi', html:'abcde', css: {'background-color':'#ff0'}}).appendTo('ul');
								// == > this.html('abcde') 把html当做函数处理	
					...
					// HANDLE: $(expr, $(...))
				} else if (!context || context.jquery) { //hzq_201501142153
	...
	3.
		...
		//深复制
		var obj1 = {name:'monkey',age:30};
		var obj2 = {friend:{name:'jjj',age:33}, mobile:'18701046848'};

		$.extend(true, obj1,obj2); //

		console.info(obj1.friend.name); // jjj

		obj2.friend.name = 'ggg';

		console.info(obj2.friend.name); // ggg
		console.info(obj1.friend.name); // jjj
		...
		console.info(obj2.friend.name); // ggg
		console.info(obj1.friend.name); // ggg
		</pre>
	 */
	...
	jQuery.extend({
		...
		isPlainObject: function(obj) {
			...
			// core_hasOwn.call( obj, key ) 属性key不为空，且属性key的对象自身的（即不是通过prototype继承的）
			var key;
			for (key in obj) {}
		...
		// keepScripts (optional): If true, will include scripts passed in the html string (true允许有script标签)
		parseHTML: function(data, context, keepScripts) {
			if (!data || typeof data !== "string") { //如果data不为空或者data不是string类型的,则返回
				return null;
			}
			...
			if (typeof data === "string") {
				...
				if (data) {
					...
					if (rvalidchars.test(data.replace(rvalidescape, "@")
						...
						// var ijson = new Function('return {"name":"hou","age":23}');
						// console.info(ijson()); 
						//好吧我承认这样写真无语 ， 我还以为是什么牛逼的方法，哎！！
		...
		merge: function(first, second) {
			//如果second为一个字符串？会出现什么情况，你猜猜??
			//这种情况会进入到var j = {'name':'aaaa'}; $.merge([],j);


			//如果second是一个字符串：$.merge([],'hello');--> ['h','e','l','l','o']
			//如果second是一个数字，即判断的第二种情况 ，此时$.merge([],2); --> [] 不会合并
			var l = second.length,
				i = first.length,
				j = 0;
			...
			if (typeof l === "number") {
				...
				//当second不具有length是会进入到该方法，同时进入该方法是要一定条件的, 思考一下什么情况下才能执行while？
			...
			//进入else的条件比较特殊，second需为包含数字的属性，且要求比较严格如{0:'aaa',1:'bbb',2:'ccc'}
			//其他不符合条件的都不会进行合并
			return first;
		},
		 ...
		 * };
		 *
		 * $(document).click(obj.show);
		 * $(document).click($.proxy(obj.show),obj);
		 */
		proxy: function(fn, context) {
			var args, proxy, tmp;

			// 此处是为了是proxy兼容$(document).click($.proxy(obj,"show"));这种形式
			...
			// Simulated bind  获取出fn/context之外的的参数保存到args中，所以从第三个开始
		...
		access: function(elems, fn, key, value, chainable, emptyGet, raw) {
			...
			if (jQuery.type(key) === "object") {
				...
				// Sets one value
			} else if (value !== undefined) { //value存在
				chainable = true;
				...
				if (bulk) { //key为空、null,undefined 时
		...
		now: function() {
			return (new Date()).getTime(); //高级浏览器中可以使用Date.now()代替，这也是2.0以后与现在版本不同之处
		}
	});
